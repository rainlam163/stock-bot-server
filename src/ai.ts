...
`;
    ...
    try {
        ...
        try {
           ...
           if (!Array.isArray(result) && Array.isArray(json)) result = json;
        } catch (e) {
...
async function predictSectors(globalData: any[], sectorData: any[], newsHeadlines: string[]): Promise<any[]> {
...
`;
    const getResultByModel = async (model: string): Promise<any[]> => {
        const completion: any = await client.chat.completions.create({
