...
async function getAIAdvice(symbol, stockName, stockHistory, indexHistory, newsList = []) {
    ...
    const indexRecent = indexHistory.slice(-20);
    ...
    const bbResults = TI.BollingerBands.calculate({ period: 20, values: closes, stdDev: 2 });
    const currentBB = bbResults[bbResults.length - 1] || { upper: 0, middle: 0, lower: 0 };
    // 5. 计算平均换手率（近5日），用于判断是否放量
    const recentTurnovers = recent.slice(-5).map(d => d.turnover);
    const avgTurnover5 = (recentTurnovers.reduce((a, b) => a + b, 0) / 5).toFixed(2);
    const volRatio = (todayVol / avgTurnover5).toFixed(2); // 计算量比
    // 6. 计算均线与价格位置 (MA5, MA12, MA72)
    const ma5 = (closes.slice(-5).reduce((a, b) => a + b, 0) / 5).toFixed(2);
    const ma12 = (closes.slice(-12).reduce((a, b) => a + b, 0) / 12).toFixed(2);
    const ma20 = (closes.slice(-20).reduce((a, b) => a + b, 0) / 20).toFixed(2);
    const ma72 = (closes.slice(-72).reduce((a, b) => a + b, 0) / 72).toFixed(2);
