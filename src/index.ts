...
import { getMarketContext, analyzeStock, analyzeStockStream, analyzeBatchStocks } from './analyzer.js'
...
app.get('/api/recommend', async (c) => {
  try {
    ...
    let filteredCandidates = hotStocks.filter((s: any) => {
      // 基础过滤
      if (s.name.includes('ST') || s.name.includes('退') || s.price < 3) return false;
      
      // 1. 剔除已经涨停的 (没机会买了)
      if (s.changePercent > 9.5) return false;
      
      // 2. 剔除极度缩量的 (无主力) - 量比需 > 0.8
      if (s.volumeRatio < 0.8) return false;
      
      // 3. 剔除僵尸股 - 换手率需 > 1.5%
      if (s.turnoverRate < 1.5) return false;
    ...
    const finalResults = aiResults.map(r => ({
       ...
       // 补充价格和涨幅信息 (从原始候选列表获取)
       changePercent: filteredCandidates.find((c: any) => c.code === r.code)?.changePercent || 0
