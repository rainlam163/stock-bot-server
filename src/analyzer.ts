...
import { getAIAdvice, getAIAdviceStream, HoldingInfo } from './ai.js'
 ...
 */
async function analyzeBatchStocks(candidates: any[]) {
    // 限制并发数，分批处理以平衡速度与稳定性
    const results = [];
    const BATCH_SIZE = 25;
    ...
    for (let i = 0; i < candidates.length; i += BATCH_SIZE) {
        ...
        const tasks = batch.map(async (stock) => {
            try {
                ...
                if (!history || !history.klines || history.klines.length < 20) return null;
