...
export async function runSelectionJob() {
    ...
    try {
        ...
        let candidates = allStocks.filter((s: any) => {
            // 1. 非 ST/退
            if (s.name.includes('ST') || s.name.includes('退')) return false;
            
            // 2. 市值过滤
            if (!s.marketCap || s.marketCap < MIN_MARKET_CAP) return false;

            // 3. 上市时间过滤 (f26)
            if (!s.listingDate || s.listingDate > date60DaysAgo) return false;
        ...
        const tasks = candidates.map((stock: any) => limit(async () => {
            try {
                ...
                if (idxT20 < 0) return null; // Not enough history
        ...
        const top20 = validResults.sort((a: any, b: any) => b.score - a.score).slice(0, 20);

        // Format for DB
        const finalOutput = top20.map((item: any, index: number) => {
...
综合得分 **${item.score.toFixed(2)}**，在全市场大市值股票中排名第 **${index + 1}**。
        ...
        // 直接存入计算出的真实得分
        const dbData = finalOutput.map((i: any) => ({
            code: i.code,
            name: i.name,
